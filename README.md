# Secure Chat App with React, Stream and Auth0

Tutorial: https://getstream.io/blog/securing-a-chat-app-with-react-and-auth0/

## Getting Started

1. Clone this repository and `cd` into it.
2. See tutorial for notes on how to get the required credentials from Auth0 and Stream.
3. Create a `.env` file and update it with your Stream credentials.
4. Open up `client/src/Chat.js` and update it with your Stream API key
5. Run `node server.js` from within the project root to start the Node server on
   port 7000.
6. Run `npm start` from the `client` directory to start the app development server. View http://localhost:3000 in your browser.

## Pre-requisites

- Have prior experience working with Node.js and React.
- Have Node.js (version 8 or later) and npm installed on your machine.

## Built With

- [React](https://reactjs.org) - For creating the application frontend
- [Stream Chat](https://getstream.io/chat/) - Chat features
- [Auth0][https://auth0.com] - User authentication

## Licence

[MIT](https://opensource.org/licenses/MIT)

